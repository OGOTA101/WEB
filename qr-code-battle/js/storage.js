...
const Storage = {
    ...
    // ... (existing getDeck, saveDeck, getStock, etc. - assume they are preserved if I don't touch them or target them correctly)
    // Wait, replacing a large chunk or appending?
    // I should append methods.
    ...
    removeFromStock(unitId) {
        ...
        // 最低4体は保持（デッキが組めなくなるため）
        if (data.stock.length <= 4) {
            return { success: false, message: 'ユニットが4体以下のため、これ以上削除できません' };
        }
