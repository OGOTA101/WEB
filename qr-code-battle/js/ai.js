...
class BattleAI {
    ...
    decideAction(unit) {
        ...
        // 遠距離ユニットの場合（射程100px以上）
        // 以前は rng >= 2 だったが、今は rng = 500 (50px) とかなので
        const rangeMultiplier = CONFIG.BATTLE.RANGE_PIXEL_MULTIPLIER || 0.1;
        if (unit.stats.rng * rangeMultiplier >= 100) {
    ...
    rangeUnitAI(unit, enemies) {
        ...
        const rangeMultiplier = CONFIG.BATTLE.RANGE_PIXEL_MULTIPLIER || 0.1;
        // 最低射程保証（AIもこれを知っていないと近づきすぎてスタックする）
        const attackRange = Math.max(unit.stats.rng * rangeMultiplier, CONFIG.BATTLE.UNIT_SIZE + 10);
